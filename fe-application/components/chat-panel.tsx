...
import { Send, Bot, Loader2, GripVertical } from "lucide-react"
...
export function ChatPanel({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        className="p-0 flex flex-col [&>div]:!max-w-none"
        style={{
          width: `${panelWidth}px`,
      ...
      >
        ...
        <div
          ref={resizeRef}
          className={cn(
            "absolute left-0 top-0 bottom-0 w-1 cursor-col-resize bg-border hover:bg-primary/50 transition-colors group",
        ...
        <SheetHeader className="p-4 border-b ml-4">
          <SheetTitle className="flex items-center gap-2">
            <Bot className="h-5 w-5 text-primary" />
            Healthcare Data Assistant
            <div className="ml-auto text-xs text-muted-foreground font-normal">{panelWidth}px wide</div>
          </SheetTitle>
        </SheetHeader>
